...
os.environ['PATH'] = os.path.dirname(r'C:\Program Files (x86)\MediaInfo.dll\MediaInfo.dll\MediaInfo_InfoTip.dll') + ';' + os.environ['PATH']
from pymediainfo import MediaInfo

TMP_FRAMES_PATH = './frames/'
...
def generate_static_thumbnail(video, output):
    ...
    subprocess.call(["ffmpeg", "-i", video,"-vf","select=gt(scene\,0.1),scale=280:-1","-frames:v","1",output])

conn = sqlite3.connect('esnap.db')
setsdb = conn.cursor()
...
for f in files:
    ...
    if os.path.isfile(f+'.jpg'):
        ...
        dir = '/esnap/'+'/'.join(f.split('\\')[3:])
        print 'dir = ', dir
        thumb = '/'.join(f.split('\\')[3:])+'.jpg'
        print 'thumb = ', thumb
        # print os.path.basename(f)
        media_info = MediaInfo.parse(f)
        date = time.strptime(media_info.tracks[0].tagged_date, '%Z %Y-%m-%d %H:%M:%S')
        print 'date = ', date
        # date = time.gmtime(os.path.getmtime(f))
        date = '20'+time.strftime('%y%m%d', date)
        # print date
        try:
            model = string.split(setname,'_')[1]#.replace('-&-','.and.').replace('.','-')
            print 'model = ', model
        except:
            # pass
            ...
            print setname
        try:
            setname = string.capwords(string.split(setname,'_')[2])
            print 'final setname = ', setname
        except:
             print 'failed! dir = ', dir
             exit()
