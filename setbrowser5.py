...
class videoWidget(QtWidgets.QMainWindow):
    ...
    def __init__(self, path, parent=None):
        ...
        self.hwnd = int(self.video.winId())
        # print self.video.hwnd
    ...
    def keyPressEvent(self, e):
        ...
        # print e.key()
        if e.key() == 16777236:
            self.player.set_time(self.player.get_time() + (1000*10))
        if e.key() == 16777234:
            self.player.set_time(self.player.get_time() - (1000*10))
...
class imageLink(QtWidgets.QWidget):
    ...
    def stopMovie(self, event):
        if self.site == "wowporn" and hasattr(self,'movie'):
            self.movie.stop()
            self.label.setPixmap(self.pixmap)
            del self.movie
...
class SetBrowser(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent = None):
        ...
        self.sites = ['errotica', 'met', 'abby', 'wowporn']
