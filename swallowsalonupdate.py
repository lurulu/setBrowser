...
def generate_static_thumbnail(video, output):
    ...
    subprocess.call(["ffmpeg", "-i", video,"-ss","18","-vf","select=gt(scene\,0.1),scale=280:-1","-frames:v","1",output])

conn = sqlite3.connect('swallowsalon.db')
setsdb = conn.cursor()
conn.execute('delete from sets')

files = []
start_dir = r'z:\\swallowsalon'
patterns   = ("*.mp4", "*.wmv")
