...
conn = sqlite3.connect('ultrafilms.db')
setsdb = conn.cursor()
conn.execute('delete from sets')

files = []
start_dir = r'z:\\ultrafilms'
patterns   = ("*.mp4", "*.wmv")
...
for f in files:
    ...
    if os.path.isfile(f+'.jpg'):
        ...
        dir = '/ultrafilms/'+setname
        thumb = setname+'.jpg'
        # print os.path.basename(f)
        date = re.search('(\d\d\.\d\d\.\d\d)', setname, flags=re.IGNORECASE).group(1)
